class FlagButtons extends React.Component {

  setFlagLanguages(languages) {
      let options = []
      languages.forEach((language, index) =>{
          options.push(
              <a key={language.country} className="dropdown-item" onClick={() => this.setLanguag(language.country)}>
                  <span className={"flag flag-" + language.country.toLowerCase()}></span> {this.listType == 'countryName' ? language.countryName : language.language}
              </a>
          )
      });
      return options;
  ...
  defaultSelection(){
      let selectedLanguage = this.languages.filter((language) => {
  ...
  setLanguag(countryCode){
    let currentLanguages = this.languages.filter((lang) => {
    ...
    let theLanguage = this.languages.filter((lang) =>{
  ...
  render(){
      return (
          <div>
              <a className="nav-link dropdown-toggle" id="dropdownDiv" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                  <span className={"flagName flag flag-" + this.defaultSelection()[0].country}> </span>
              ...
              <div className="dropdown-menu" aria-labelledby="dropdownDiv">
  ...
  renderCountryFlags(element, languages, props){
  ...
  renderLanguageFlags(element, languages, props){
    this.element = (this.element ? this.element: element);
    this.props = (this.props ? this.props : props)
  ...
  setCountryList(defaultLanguages){
  ...
  setLanguageList(defaultList){
    ...
    this.setSelectionList();
      ...
      this.selectionList.filter((countryLanguages) => {
        if(countryLanguages.default){
          this.tempList.push(countryLanguages);
        }
      })
      if(defaultList && defaultList.length > 0){
        defaultList.forEach((lang) => {

          this.tempList.filter((filtered) => {
  ...
  validateSelector(defaultLanguages, listType){
